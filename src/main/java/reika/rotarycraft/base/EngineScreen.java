...
public abstract class EngineScreen<E extends BlockEntityEngine, T extends CoreContainer<E>> extends NonPoweredMachineScreen<E, T> {
    ...
    protected void extractLabels(GuiGraphicsExtractor stack, int pMouseX, int pMouseY) {
        if (engine.getEngineType().burnsFuel()) {
            ...
            // 1.21.5: Screen#renderTooltip(GuiGraphics, int, int) was renamed/removed; tooltip extraction now via extractTooltip pipeline.
            int dx = this.getFuelBarXPos();
            int dy = this.getFuelBarYPos();
            if (api.isMouseInBox(j + dx, j + dx + this.getFuelBarXSize(), k + dy, k + dy + this.getFuelBarYSize(), pMouseX, pMouseY)) {
