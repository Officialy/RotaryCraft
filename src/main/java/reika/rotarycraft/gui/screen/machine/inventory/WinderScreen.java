...
public class WinderScreen extends GuiOneSlotScreen<BlockEntityWinder, WinderContainer> {
    ...
    public void init() {
        ...
        if (input)
            addRenderableWidget(new Button.Builder(Component.literal("Input Mode"), this::actionPerformed).pos(var5 + imageWidth / 2 - 35, var6 + imageHeight / 2 - 26).size(65, 20).build());
        else
            addRenderableWidget(new Button.Builder(Component.literal("Output Mode"), this::actionPerformed).pos(var5 + imageWidth / 2 - 35, var6 + imageHeight / 2 - 26).size(65, 20).build());
    }
