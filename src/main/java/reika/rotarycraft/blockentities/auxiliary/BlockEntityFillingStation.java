...
//        @Override
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        ...
        if (i == FUEL_SLOT && itemstack.getCapability(ForgeCapabilities.FLUID_HANDLER_ITEM).isPresent()) {

            boolean hasFluid = itemstack.getCapability(ForgeCapabilities.FLUID_HANDLER_ITEM).map(e -> !e.getFluidInTank(0).isEmpty()).orElse(false);
            return hasFluid || itemstack.getItem() == RotaryItems.ETHANOL.get();
//            return true;
        }
        return false;
    }
