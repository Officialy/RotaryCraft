...
/**
 * Crystallizer freezing: {@code amount} mB of a fluid freezes into an item once the machine is
 * chilled below the fluid's freezing point (water -> ice, lava -> stone, ethanol -> crystals).
 * Data-driven port of the legacy {@code RecipesCrystallizer}. The input is a fluid, so matches()
 * is fluid-driven (the SingleRecipeInput item is unused).
 */
public class CrystallizerRecipe implements Recipe<SingleRecipeInput> {
    ...
    public CrystallizerRecipe(Holder<Fluid> fluid, int amount, Holder<Item> output, int count) {
        this.fluid = fluid;
        this.amount = amount;
        this.output = output;
        this.count = count;
    ...
    public RecipeSerializer<? extends Recipe<SingleRecipeInput>> getSerializer() {
        return RotaryRecipeSerializers.CRYSTALLIZER.get();
    }

    @Override
    public RecipeType<? extends Recipe<SingleRecipeInput>> getType() {
        return RotaryRecipeTypes.CRYSTALLIZER.get();
    }
