...
public class RoCModelProvider extends ModelProvider {
    ...
    protected void registerModels(BlockModelGenerators blockModels, ItemModelGenerators itemModels) {
        ...
        for (var holder : RotaryBlocks.BLOCKS.getEntries()) {
            ...
            // Pipe-shell blocks (fluid pipe, hose, fuel line, separation, suction, bedrock pipe) are
            // drawn entirely by the PipeRenderer BER (shared PipeShellRenderer flat-shell geometry);
            // their hand-authored blockstate (assets/rotarycraft/blockstates/<type>.json) points at an
            // empty particle-only model (models/block/pipe/<type>/shell.json). Skip the blockStateOut
            // emission entirely (the hand file is authoritative — emitting our own would conflict).
            // Point blockModelId at the type's closed core model, kept purely as the inventory icon,
            // and fall through to the normal item-icon code below so the icon (and ModelProvider's
            // own missing-model validation) both resolve correctly.
