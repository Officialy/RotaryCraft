...
public enum PowerReceivers {
    ...
    //PILEDRIVER(80000, 1, BlockEntityPileDriver.BASEPOWER), //4000kg hammer * 10 m/s^2 / 0.5m spool
    PLAYERDETECTOR(),
    PROJECTOR(512),
    PULSEJET(1, 131072, 1),
    PUMP(8, 1, 1024),
     ...
     */
    PowerReceivers(int T, int S, int P) {
        minT = T;
        minS = S;
        minP = P;
        powers = null;
    ...
    PowerReceivers(int[] T, int[] S, long[] P) {
...
//    }
    ...
    public static PowerReceivers initialize(MachineRegistry m) {
        if (m.isPowerReceiver()) {
            ...
            String em = m.toString();
            if (name == null)
                throw new RegistrationException(RotaryCraft.getInstance(), m + " does not correspond to an existing machine Enum!");
            for (PowerReceivers e : PowerReceivers.list) {
                String en = e.toString();
                if (en.equals(em)) {
                    return e;
    ...
    public MachineRegistry getMachine() {
        ...
        RotaryCraft.LOGGER.error("This should not happen! Power Receiver " + this + " has no machine!");
        return null;
    }
